...
-- Task: Filter grouped results using HAVING
SELECT customer_id, SUM(total_amount) AS total_spent
FROM orders
GROUP BY customer_id
HAVING SUM(total_amount) > 200;

-- Task: Use WHERE and HAVING together
SELECT customer_id, SUM(total_amount) AS total_spent
FROM orders
...
-- Task1: Use JOIN statements to combine data from multiple tables 
-- INNER JOIN - Display all orders with customer information
SELECT orders.id, customers.first_name, customers.last_name, orders.order_date, orders.total_amount
FROM orders
INNER JOIN customers ON orders.customer_id=customers.id;

-- LEFT JOIN - Display all orders, including orders without a customer
SELECT orders.id, customers.first_name, customers.last_name, orders.order_date, orders.total_amount
FROM orders
LEFT JOIN customers ON orders.customer_id=customers.id;

-- RIGHT JOIN - Display all customers, including those without orders
SELECT orders.id, customers.first_name, customers.last_name, orders.order_date, orders.total_amount
FROM orders
